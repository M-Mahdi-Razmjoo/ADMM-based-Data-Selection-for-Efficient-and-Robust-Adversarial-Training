...
# === Hyperparameters (Identical to other scripts) ===
BATCH_SIZE = 256
LEARNING_RATE = 0.1
MOMENTUM = 0.9
WEIGHT_DECAY = 5e-4
...
SEEDS = [42, 43, 44]
NUM_CLASSES = 100
METHOD_NAME_RANDOM = f"Random_CIFAR100_k{K_SAMPLES}_bs{BATCH_SIZE}"
OUT_DIR = "results_random_runs"
os.makedirs(OUT_DIR, exist_ok=True)

print(f"Method: {METHOD_NAME_RANDOM}, K={K_SAMPLES}, BATCH_SIZE={BATCH_SIZE}, seeds={SEEDS}")
...
# === Core Training Logic for Random Selection ===
def train_epoch_random_collect(model, optimizer, data_loader, k):
    model.train()
    selected_history = []
    ...
    for clean_images, labels in data_loader:
        ...
        # We need to select k indices out of the total pool (2 * batch_size)
        total_samples = combined_images.size(0)
        # Use torch.randperm to get random indices
        sel_idx = torch.randperm(total_samples, device=device)[:k]
        
        # Track history (for consistency with other scripts, though stability will be low)
    ...
    # Calculate stability (expected to be near stochastic average)
    mean_overlap = 1.0 # Placeholder matching other scripts
    stability = compute_selection_stability(selected_history, k)
    return mean_overlap, stability

# === Experiment Loop ===
def run_experiment_seed_random(seed, method_name=METHOD_NAME_RANDOM):
    torch.manual_seed(seed)
    np.random.seed(seed)
    if torch.cuda.is_available(): torch.cuda.manual_seed_all(seed)

    print(f"\n=== Random Selection RUN seed {seed} ===")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # Run Random Training Epoch
        mean_overlap, stability = train_epoch_random_collect(model, optimizer, train_loader, K_SAMPLES)
...
for s in SEEDS:
    fname, out = run_experiment_seed_random(s, method_name=METHOD_NAME_RANDOM)
    seed_files_random.append(fname)
    seed_outputs_random.append(out)

# === Aggregation ===
min_epochs = 0 if not seed_outputs_random else min(len(o['training_history']['epoch']) for o in seed_outputs_random)
...
for m in metrics:
    arr = np.array([o['training_history'][m][:min_epochs] for o in seed_outputs_random], dtype=float)
    agg_history[f'{m}_mean'] = list(np.nanmean(arr, axis=0))
    agg_history[f'{m}_std']  = list(np.nanstd(arr, axis=0, ddof=1))

per_class_std = np.array([o['training_history']['per_class_std_acc'][:min_epochs] for o in seed_outputs_random], dtype=float)
per_class_rob = np.array([o['training_history']['per_class_robust_acc'][:min_epochs] for o in seed_outputs_random], dtype=float)

aggregate_output = {
    'experiment_name': f"{METHOD_NAME_RANDOM}_aggregate",
    'seed_files': seed_files_random,
    'hyperparameters': {
        'batch_size': BATCH_SIZE, 'learning_rate': LEARNING_RATE, 'epochs': EPOCHS,
        'k_percentage': K_PERCENTAGE, 'k_samples': K_SAMPLES, 'epsilon': EPSILON
    },
    ...
    'final_summary_aggregate': {
        'final_std_acc_mean': float(np.nanmean([o['final_summary']['final_std_acc'] for o in seed_outputs_random if o['final_summary']['final_std_acc'] is not None])),
        'final_std_acc_std' : float(np.nanstd([o['final_summary']['final_std_acc'] for o in seed_outputs_random if o['final_summary']['final_std_acc'] is not None], ddof=1)),
        'final_robust_acc_mean': float(np.nanmean([o['final_summary']['final_robust_acc'] for o in seed_outputs_random if o['final_summary']['final_robust_acc'] is not None])),
        'final_robust_acc_std' : float(np.nanstd([o['final_summary']['final_robust_acc'] for o in seed_outputs_random if o['final_summary']['final_robust_acc'] is not None], ddof=1)),
        'total_training_time_mean': float(np.nanmean([o['final_summary']['total_training_time'] for o in seed_outputs_random if o['final_summary']['total_training_time'] is not None])),
        'total_training_time_std' : float(np.nanstd([o['final_summary']['total_training_time'] for o in seed_outputs_random if o['final_summary']['total_training_time'] is not None], ddof=1))
    }
}

agg_fname = os.path.join(OUT_DIR, f"{METHOD_NAME_RANDOM}_aggregate.json")
with open(agg_fname, 'w') as f:
    json.dump(aggregate_output, f, indent=4)

print("Saved aggregate results to", agg_fname)
print("Seed files:", seed_files_random)